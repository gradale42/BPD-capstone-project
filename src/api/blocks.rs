...
pub async fn get_blocks(
    ...
    match mode {
        "index" => {
            // database query
            let total_blocks = match state.block_service.count_blocks_in_db().await {
            ...
            let blocks = match state.block_service.get_blocks_from_db(length as i64, start as i64, &order_by).await {
                Ok(b) => b,
                Err(e) => {
