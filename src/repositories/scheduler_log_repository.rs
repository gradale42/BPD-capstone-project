...
pub trait SchedulerLogRepository: Send + Sync {
    ...
    async fn update_log_success(&self, conn: &mut PgConnection, id: Uuid, result: &SyncResult) -> Result<(), Error>;
    async fn update_log_failure(&self, conn: &mut PgConnection, id: Uuid, error: &str) -> Result<(), Error>;
    async fn list_logs(&self, conn: &mut PgConnection, limit: i64, offset: i64) -> Result<Vec<SchedulerLog>, Error>;
...
impl SchedulerLogRepository for PostgresSchedulerLogRepository {
    ...
    async fn count_logs(&self, conn: &mut PgConnection) -> Result<i64, Error> {
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM scheduler_log")
            .fetch_one(conn)
            .await?;
        Ok(row.0)
    }
